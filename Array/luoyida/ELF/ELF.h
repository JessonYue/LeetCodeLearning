...
void program_header_64_parse(Elf64_Phdr *ehdr, int count);

#define INF_ET_NONE "No file type(未知文件类型)"
#define INF_ET_REL "Relocatable file(可重定位文件)"
#define INF_ET_EXEC "Executable file(可执行文件)"
#define INF_ET_DYN "Shared object file(共享目标文件.so文件)"
#define INF_ET_CORE "Core file(内核文件)"
#define INF_ET_NUM "Number of defined types"
#define INF_ET_LOOS "OS-specific range start"
#define INF_ET_HIOS "OS-specific range end"
#define INF_ET_LOPROC "Processor-specific range start(特定处理器文件扩展下边界)"
#define INF_ET_HIPROC "Processor-specific range end(特定处理器文件扩展上边界)"
