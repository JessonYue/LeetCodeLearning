...
LinkedList_ *inset(LinkedList_ *list, int index, int data);

bool isPalindrome(LinkedList_ *list);
...
bool isPalindrome(LinkedList_ *list) {
    int preposition = 0;
    int posterior = list->length+1;
    for (int i = 0; i <list->length/2 ; ++i) {
        Node_ *head = get(list,preposition);
        Node_ *last = get(list,posterior);
